...
if 'player_df' in globals() and 'team_df' in globals():
    ...
    # NEW Visualization 9: Additional Subplot Example
    fig, axs = plt.subplots(1, 3, figsize=(18, 6))

    # Line plot example - Assuming year-wise player count
    years = np.arange(2008, 2024)
    players_count = np.random.randint(100, 200, len(years))  # Replace with actual data if available
    axs[0].plot(years, players_count, marker='o', linestyle='-', color='blue')
    axs[0].set_title("Year-wise Player Count", fontsize=12)
    axs[0].set_xlabel("Year", fontsize=10)
    axs[0].set_ylabel("Number of Players", fontsize=10)

    # Age distribution using line plot
    age_distribution.plot(kind='line', ax=axs[1], color='green', marker='x')
    axs[1].set_title("Age Distribution Line Plot", fontsize=12)
    axs[1].set_xlabel("Age", fontsize=10)
    axs[1].set_ylabel("Count", fontsize=10)

    # Scatter plot example
    axs[2].scatter(age_counts['Age'], age_counts['Count'], color='red', alpha=0.5)
    axs[2].set_title("Age vs Count Scatter", fontsize=12)
    axs[2].set_xlabel("Age", fontsize=10)
    axs[2].set_ylabel("Count", fontsize=10)

    plt.tight_layout()
    plt.savefig('additional_subplots.png')
    plt.show()
